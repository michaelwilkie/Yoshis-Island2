...
vector<std::string> stringsplit_n(std::string str, std::string delimiter, int n)
{
	...
	std::string token;
	vector<std::string> result;
	for (size_t pos = 0; ((pos = str.find(delimiter)) != std::string::npos) && n > 0;n--)
	{
		token = str.substr(0, pos);
		result.push_back(token);
		str.erase(0, pos + delimiter.length());
...
////////////////////////////////////////////////////////////////////////
int consoleCommand(std::string argstr)
{
	// to do:
...
void modifyEntity(Interactive *i, std::string key, std::string value)
{
	...
	switch (ktype)
	{
		...
		case KEYVALUE::DIMENSIONS:
		{
			...
			SDL_Rect d{ i->loc.x, i->loc.y, std::stof(argvect[0]), std::stof(argvect[1]) };
			i->dim.push_back(d);
			break;
		}
		...
		{
			vector<std::string> argvect = stringsplit(value, " ");
			if (argvect.size() < 1 || argvect.size() > 1)
			{
				cout << "Invalid value: " << value << endl;
				cout << "Usage: x" << endl;
				return;
			}
			i->setSpeed(std::stof(argvect[0]));
			break;
		}
...
Interactive* createEntity(POINT loc, int layer, ENTTYPE type)
{
	...
	case ENTTYPE::ELEVATOR_TYPE:
		...
		break;
	case ENTTYPE::TRIGGER_TYPE:
		i = new Trigger(loc.x, loc.y);
		break;
	}
	//e = i;
...
//////////////////////////////////////////////////////////////////////////////
// Retrieve the reference/pointer of an entity based on the ID that's given //
// nullptr is returned if the ID does not exist                             //
//////////////////////////////////////////////////////////////////////////////
Interactive* getEntity(int id)
{
